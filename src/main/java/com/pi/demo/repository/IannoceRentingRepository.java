...
//	public int reclamationNNTraitees();
    ...
    @Modifying
    @Transactional
    @Query("UPDATE Annonce e SET e.favoriteAnnonce=1 where e.id=:annonceId AND type_annoce= 'renting'")
    public void favouriteAnnonceJPQL( @Param("annonceId")int annonceId);
    ...
    @Query("SELECT u FROM Annonce u WHERE u.AdState ='Accepted' AND type_annoce= 'renting' ORDER BY u.price ASC")
